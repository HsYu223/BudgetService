        ...
        public decimal Query(DateTime start, DateTime end)
        {
            ...
            while (tmp <= end)
            {
                ...
                if (tmp.ToString("yyyyMM") == startTYarMonth.ToString())
                {
                    ...
                    var monthAmount = totalBudget.Where((x => x.YearMoth.Equals(tmp.ToString("yyyyMM")))).Select(x => x.Amount).FirstOrDefault();
                ...
                {
                    tmpAmount += Convert.ToDecimal(totalBudget.Where(x => x.YearMoth.Equals(tmp.ToString("yyyyMM"))).Select(x => x.Amount).FirstOrDefault());
                }
